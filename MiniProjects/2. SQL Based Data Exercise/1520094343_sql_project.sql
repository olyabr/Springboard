...
Code:
SELECT DISTINCT CONCAT(members.firstname, ' ', members.surname) AS concat_name,
	   sub.faname
FROM (
    SELECT facilities.facid as facid,
           facilities.name as faname,
...
WHERE CONCAT(members.firstname, ' ', members.surname) NOT LIKE '%GUEST%'
AND sub.facid < 2
...
Answer:
facility name    member name    cost
...
Code:
SELECT facilities.name as 'facility name',
	   CONCAT(sub.firstname,' ', sub.surname) as 'member name',
	   CASE WHEN sub.memid = 0 THEN sub.slots*facilities.guestcost
            ELSE sub.slots*facilities.membercost 
			END AS cost
FROM (SELECT members.firstname,
             members.surname,
      		 bookings.memid,
      		 bookings.slots,
             bookings.facid,
...
WHERE LEFT(sub.starttime,10) = '2012-09-14'
AND CASE WHEN sub.memid = 0 THEN sub.slots*facilities.guestcost
            ELSE sub.slots*facilities.membercost 
			END > 30
ORDER BY cost DESC
